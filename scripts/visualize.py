...
class Animation:
  def __init__(self, map, schedule):
    ...
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      name = d["name"]
      self.agents[name] = Rectangle((d["start"][0] - 0.3, d["start"][1] - 0.3), 0.6, 0.6, facecolor=Colors[0], edgecolor='black')
      ...
      self.agent_names[name] = self.ax.text(d["start"][0], d["start"][1], name.replace('cube', ''))
  ...
  def animate_func(self, i):
    for agent_name, agent in self.combined_schedule.items():
      ...
      p = (pos[0], pos[1])
      self.agents[agent_name].xy = (p[0] - 0.3, p[1] - 0.3)
    ...
    for i in range(0, len(agents_array)):
      for j in range(i+1, len(agents_array)):
        d1 = agents_array[i]
        d2 = agents_array[j]
        pos1 = np.array(d1.xy) - 0.3
        pos2 = np.array(d2.xy) - 0.3
...
if __name__ == "__main__":
  ...
  map = rospy.get_param("/cbs_input")
  path = rospy.get_param("/cbs_output/schedule/cube1")
  schedule = rospy.get_param("/cbs_output")
  # rospy.logerr(rospy.myargv()[1:])
  
  for i in path:
    goal = np.array([i['x'], i['y']] ) * 12
