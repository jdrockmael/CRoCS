...
def drive(effort : Float32MultiArray):
    left, right = effort.data
    drive_one_wheel(left, True)
    drive_one_wheel(right, False)

if __name__ == '__main__':
    rospy.init_node('vel_reader')
    init()
    rospy.Subscriber("wheel_eff", Float32MultiArray, drive)
    monitor_vel()
