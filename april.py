...
import rospy
from april import CamTest

if __name__ == "__main__":
    rospy.init_node("cam_test")
    cam = CamTest()
    #rospy.on_shutdown()
    rospy.loginfo("Camera is now started, ready to get commands.")
    rospy.spin()
...
class CamTest:
    ...
    if not ardu:
        ...
        # draw a crosshair
        def plotPoint(self, image, center, color):
            center = (int(center[0]), int(center[1]))
            image = cv2.line(image,
                            (center[0] - self.LINE_LENGTH, center[1]),
                            (center[0] + self.LINE_LENGTH, center[1]),
                            color,
                            3)
            image = cv2.line(image,
                            (center[0], center[1] - self.LINE_LENGTH),
                            (center[0], center[1] + self.LINE_LENGTH),
                            color,
                            3)
            return image
