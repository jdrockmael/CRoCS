...
april_ekf = ekf.RobotEKF(95.25)
u = np.array([0, 0])

landmark = np.array([0, 190])

t = time()
while(1):
    dt = time() - t
    april_ekf.predict(u, dt)
    t = time()
    # Take each frame
    _, frame = cap.read()
    ...
    if tags:
        ...
        for tag in tags:
            ...
            hor_distance = pose2real(tag.pose_t[0]) # 25.4mm is the real world distance from center of april tag to edge
            ...
            z = np.array([[avg_range],
                [np.deg2rad(avg_bearing) ]])
            april_ekf.update(z, landmark, dt)

            print(april_ekf.pose)
